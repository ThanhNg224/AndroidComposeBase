...
### Added
- **New published module `AndroidCoreBase-ui-compose`**: `AndroidComposeBaseTheme`, `ComposeView.setThemedContent`, and `BaseComposeActivity` moved out of the main artifact into this optional, Compose-only module.
...
- **Compose Interop Support**: `AndroidComposeBaseTheme` bridges XML theme token colors to Compose `MaterialTheme`; `ComposeView.setThemedContent()` handles safe disposal; `BaseComposeActivity` for Compose screens.
...
### Changed
- **Rebranded project**: Package namespace modernized to `com.thanhng224.androidcomposebase.core` and artifact ID to `AndroidCoreBase`.
