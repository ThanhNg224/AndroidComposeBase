...
| Main-artifact Compose helpers (`AndroidComposeBaseTheme`, `ComposeView.setThemedContent`, `BaseComposeActivity`) | Moved to the new, optional `AndroidCoreBase-ui-compose` published module/coordinate — add it only if you write Compose screens. |
...
7. **Add Compose separately** if you use `AndroidComposeBaseTheme`/`setThemedContent`/`BaseComposeActivity`:
...
- `:core`'s minSdk (24) and package name (`com.thanhng224.androidcomposebase.core`).
