...
Wiring `AndroidComposeBaseTheme` and `ComposeView.setThemedContent` into `:core` compiled cleanly,
...
(`TextAppearance.AndroidComposeBase.BodyEmphasis`/`BodyMedium`/`Micro`) that exist *only* to
...
- Core package moved to `com.thanhng224.androidcomposebase.core` across `src/main`, `src/test`, and `src/testFixtures`.
- App sample package moved to `com.thanhng224.androidcomposebase` across `src/main`, `src/test`, and `src/androidTest`.
- Themes and text styles renamed to `Theme.AndroidComposeBase`, `Base.Theme.AndroidComposeBase`, and `TextAppearance.AndroidComposeBase.*`.
...
- `AndroidComposeBaseTheme` (`core/ui/theme/ComposeTheme.kt`) bridges the XML M3 theme into a Compose
...
- `ComposeView.setThemedContent()` (`core/ui/base/ComposeInterop.kt`) sets
  ...
  view.
- `BaseComposeActivity` (F14) now has real substance: `setContent { AndroidComposeBaseTheme { Content() } }`,
...
`AndroidComposeBaseTheme`, confirmed by screenshot on the physical device in both light and dark —
