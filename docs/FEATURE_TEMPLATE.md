...
```text
feature/<name>/
  navigation/       # @Serializable NavKey route, EntryProviderScope entry, optional tab metadata
  presentation/
    ui/             # Composable destination and stateless content where useful
...
val ProfileDestination = TopLevelDestination(
    id = "profile", // stable and unique; saved tab restoration uses this ID
    key = ProfileRoute,
    selectedIconRes = R.drawable.ic_nav_profile_filled,
    unselectedIconRes = R.drawable.ic_nav_profile_outlined,
...
Register `profileEntry()` in the `entryProvider<NavKey>` block in `presentation/MainShell.kt`. For a top-level destination, add `ProfileDestination` to its ordered destination list there as well. Keep labels localized. A non-tab destination needs the route and entry, but does not need `TopLevelDestination` metadata. `MainShell` keeps a separate back stack per top-level destination, so use a stable destination ID and avoid rebuilding navigation state in the feature.

## Reference examples

- `feature/onboarding` owns its completion contract, persistence key, and storage implementation. `AppRoot` observes completion and switches from onboarding to `MainShell`.
- `feature/settings` adapts `ThemeManager` and `LocaleManager`; its ViewModel calls the focused settings repository directly, while the domain language contract uses string tags and presentation maps configured tags to `AppLanguage` display resources.
- `sample/demo` uses its repository directly for count persistence and weather refresh, and keeps `IncrementCounterUseCase` for the named counter rule. Room remains the weather read source of truth.
...
3. Declare the `@Serializable` Navigation 3 route key and `EntryProviderScope<NavKey>` entry in the feature's `navigation/` package. For a top-level destination, also declare its `TopLevelDestination` metadata there with a stable, unique `id` that remains unchanged across releases; saved tab restoration depends on it. Then register the feature entry and add the destination to `presentation/MainShell.kt` in display order. Keep labels in string resources for every supported locale. `AppRoot` handles startup and onboarding; `MainShell` assembles navigation after the app is ready.
