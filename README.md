...
- **Clone the starter** — fork/clone the whole repo, rename the package/application ID, delete the `sample/` reference code, and build your app directly on top of `:app` and `:core`. You own the app's Hilt graph already (see `AndroidComposeBaseApplication.kt`, `app/src/main/java/com/example/androidcorebase/di/`).
...
- **Application Class**: Renames `AndroidComposeBaseApplication.kt` and its Manifest / DI / Kover references to `<ProjectName>Application`.
...
dependencies {
    ...
    // Optional: only if you write Compose screens against AndroidComposeBaseTheme/setThemedContent.
...
`AndroidComposeBaseTheme`/`ComposeView.setThemedContent` — not only where you first add it. The Compose
...
> `TextAppearance.AndroidComposeBase.*` / `Theme.AndroidComposeBase.*` naming.
...
│
├── core/ui-compose/                                     # Optional published Compose interop (AndroidComposeBaseTheme, setThemedContent)
│
...
└── app/                                                 # Application shell, own Hilt graph, and sample/reference code
    └── src/main/java/com/example/androidcorebase/
        ├── AndroidComposeBaseApplication.kt                 # Application entry point; plants logging, launches bounded startup
